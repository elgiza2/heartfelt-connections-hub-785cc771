...
- [x] Own-providers-only fallback: chat-fast and chat-alibaba fall back to our cloud agents (Browser Use Cloud then Hyperbrowser, `_shared/agentFallback.ts`, re-streamed as OpenAI-compatible SSE) whenever no Model Studio key works — no third-party chat gateway is used; verified live for guests; second-account routing stays optional (`VITE_COMPUTE_*`)
